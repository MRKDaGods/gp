...
from athar.core.data_models import GlobalTrajectory, Tracklet
from athar.components.associators.global_trajectories import merge_tracklets_to_trajectories
...
def aflink_post_association(
    ...
    *,
    ...
    for pass_idx in range(max_passes):
        ...
        logger.info(
            "AFLink pass %s: %s trajectory merges accepted (%s trajectories remain)",
            pass_idx + 1,
            merged_this_pass,
            len(current),
        )
    ...
    logger.info(
        "AFLink complete: %s additional merges applied across %s trajectories",
        total_merges,
        len(current),
    )
    return current
