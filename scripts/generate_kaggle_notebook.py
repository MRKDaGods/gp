...
def build_10c():
    ...
    cells.append(code(
        ...
        f"if not GT_INPUT.exists():\n"
        f"    # dataset_sources may not mount via API push — download directly\n"
        f"    import subprocess as _sp2\n"
        f"    _gt_dl = Path(\"/tmp/gt_dl\")\n"
        f"    _gt_dl.mkdir(parents=True, exist_ok=True)\n"
        f"    print(f\"GT not at {{GT_INPUT}} — downloading via kaggle API ...\")\n"
        f"    _r2 = _sp2.run(\n"
        f"        [\"kaggle\", \"datasets\", \"download\", \"{OWNER}/{SLUG_GT}\",\n"
        f"         \"-p\", str(_gt_dl), \"--unzip\"],\n"
        f"        capture_output=True, text=True\n"
        f"    )\n"
        f"    print(_r2.stdout); print(_r2.stderr)\n"
        f"    GT_INPUT = _gt_dl / \"gt_annotations\"\n"
        ...
        f"    print(f\"WARNING: GT not found anywhere, GT_DIR={{GT_DIR!r}}\")\n"
        f"print(f\"\\u2713 Checkpoint extracted -- run: {{RUN_NAME}}\")\n"
