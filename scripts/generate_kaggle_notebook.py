...
def build_10b():
    ...
    cells.append(code(
        ...
        f"\n"
        f"# Diagnose what Kaggle has mounted so far\n"
        f"import os\n"
        f"print(\"=== /kaggle/input contents ===\")\n"
        f"for p in sorted(Path(\"/kaggle/input\").iterdir()):\n"
        f"    files = [f for f in p.rglob(\"*\") if f.is_file()]\n"
        f"    top3 = [str(f.relative_to(p)) for f in files[:3]]\n"
        f"    print(f\"  {{p.name}}/  ({{len(files)}} files)  {{top3}}\")\n"
        f"\n"
        ...
        f"\n"
        f"print(f\"\\nLooking for checkpoint in: {{PREV_INPUT}}\")\n"
        f"print(\"Contents:\", sorted([f.name for f in PREV_INPUT.rglob(\"*\") if f.is_file()][:10]) if PREV_INPUT.exists() else \"directory not found\")\n"
        f"\n"
