"""
v73: Update 10c scan cell for appearance_weight + DBA on/off + top_k sweep.
"""
import json
...
new_source = '''# ============================================================
# v73: Appearance weight + DBA on/off + top_k sweep
# Baseline v72: mtmc_idf1=78.28% (PCA 384, intra 0.80/30)
# PCA 512 HURT (-0.6pp) -> staying on PCA 384.
# ============================================================
SCAN_ENABLED = True
...
if SCAN_ENABLED:
    ...
    # Phase 1: Appearance weight sweep
    for app_w in [0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]:
        st_w = round(1.0 - app_w, 4)
        configs.append({
            "tag": f"app_{app_w:.2f}",
            "appearance_w": app_w,
            "st_w": st_w,
    ...
    # Phase 2: DBA on/off (with app_w=0.75 baseline)
    configs.append({
        "tag": "dba_off",
        "appearance_w": 0.75,
        "st_w": 0.25,
    ...
    # Phase 3: mutual_nn top_k (graph density)
    ...
    print(f"v73 scan: {len(configs)} configs (8 app_w + 1 DBA_off + 5 top_k)")
    ...
    for cfg in configs:
        ...
        cmd_scan = [
            ...
            "--override", f"stage4.association.query_expansion.dba={str(cfg['dba']).lower()}",
            ...
            "--override", f"stage4.association.weights.vehicle.hsv=0.0",
    ...
    for r2 in results:
        d = "ON" if r2["dba"] else "OFF"
        print(f"{r2['tag']:<15} {r2['appearance_w']:<7} {r2['st_w']:<7} {d:<5} {r2['top_k']:<6} "
    ...
    for r2 in sorted([r for r in results if r["tag"].startswith("app_")], key=lambda x: x["appearance_w"]):
        print(f"  app_w={r2['appearance_w']:<5} -> mtmc_idf1={r2['mtmc_idf1']:.4f}  IDF1={r2['IDF1']:.4f}  ids={r2['IDSW']}")

    print("\\n--- DBA sensitivity ---")
    baseline = [r2 for r2 in results if r2["tag"] == "app_0.75"]
    dba_off = [r2 for r2 in results if r2["tag"] == "dba_off"]
    if baseline and dba_off:
        print(f"  DBA ON:  mtmc_idf1={baseline[0]['mtmc_idf1']:.4f}")
        print(f"  DBA OFF: mtmc_idf1={dba_off[0]['mtmc_idf1']:.4f}  delta={dba_off[0]['mtmc_idf1']-baseline[0]['mtmc_idf1']:+.4f}")

    print("\\n--- TOP_K sensitivity ---")
    for r2 in sorted([r for r in results if r["tag"].startswith("topk_")], key=lambda x: x["top_k"]):
...
print(f"Done! Updated scan cell with v73 sweep (14 configs)")
