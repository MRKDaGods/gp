...
\author{Abdelrahman Mostafa, Mark Ashraf, and Ali ElSayed}
...
We evaluate the vehicle pipeline on CityFlowV2 and the person pipeline on WILDTRACK. The original CityFlow benchmark introduced by Tang \emph{et al.} contains 40 cameras and 666 vehicle identities \cite{cityflow}. The later CityFlowV2 / AI City Challenge Track-1 setup used in our work expands to 46 cameras across 16 intersections in a mid-sized US city \cite{aicity2022}. Our experiments do not use that full 46-camera benchmark. They use only the S01+S02 evaluation subset, which comprises six cameras and roughly 128 vehicle identities, because that is the subset exported and evaluated by the current Kaggle notebook chain. WILDTRACK consists of seven synchronized HD cameras with overlapping fields of view and dense pedestrian interactions \cite{wildtrack}.
...
Table~\ref{tab:ablation} presents the cumulative vehicle ablation, and Figure~\ref{fig:cumulative_ablation} plots the same progression. The baseline connected-components operating point starts near 74.0\% MTMC IDF1. The largest single gain comes from FIC whitening, which contributes roughly two percentage points by compensating for camera-specific distortions in the feature space. Power normalization adds another 0.5 points, and AQE adds about 0.9 points. Conflict-free connected components, intra-camera merge, and the tracker-side change to $\texttt{min\_hits}=2$ then provide smaller but consistent gains. A separate controlled ON/OFF study shows that temporal overlap contributes about 0.9 percentage points as well, but we keep it outside the cumulative table because that measurement was taken on a different branch and is therefore not strictly additive. Taken together, these improvements lift the historical best to 78.4\%, or about 4.4 percentage points above the basic baseline. The current reproducible branch retains most of that gain at 77.5\%.
...
\begin{figure}[t]
\centering
\IfFileExists{figures/association_saturation.png}{\includegraphics[width=\linewidth]{association_saturation.png}}{\placeholderfigure{Association saturation across 225+ Stage-4 configurations.}{2.0in}}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/map_vs_mtmc.png}{\includegraphics[width=\linewidth]{map_vs_mtmc.png}}{\placeholderfigure{mAP versus MTMC IDF1 across key vehicle feature variants.}{2.0in}}
...
\begin{figure*}[t]
\centering
\IfFileExists{figures/dead_end_waterfall.png}{\includegraphics[width=0.85\textwidth]{dead_end_waterfall.png}}{\placeholderfigure{Dead-end impact waterfall for the main negative vehicle results.}{2.2in}}
...
\begin{table*}[t]
\centering
\caption{Catalog of failed approaches across the vehicle and person pipelines. Most rows are CityFlowV2 vehicle MTMC results; the global-optimal tracker row is the corresponding WILDTRACK person-pipeline dead end.}
\label{tab:dead_ends}
\input{tables/dead_ends}
\end{table*}
...
\section{Conclusion}
This paper presented a complete empirical study of a seven-stage MTMC pipeline across two domains. On CityFlowV2 vehicle tracking, a single CLIP-pretrained TransReID ViT-B/16 with FIC, power normalization, PCA, AQE, and conflict-free connected-components association reaches 77.5\% MTMC IDF1, or 91.3\% of the 2022 state of the art. On WILDTRACK pedestrian tracking, the same pipeline pattern reaches 94.7\% ground-plane IDF1. Across 225+ CityFlowV2 association configurations and 59 WILDTRACK tracker configurations, we find that the major remaining limitation is not the association stage but the quality of cross-camera invariant features.
...
\bibitem{aicity2022}
M. Naphade, S. Wang, D. C. Anastasiu, Z. Tang, M.-C. Chang, Y. Yao, L. Zheng, M. S. Rahman, A. Venkatachalapathy, A. Sharma, Q. Feng, V. Ablavsky, S. Sclaroff, P. Chakraborty, A. Li, S. Li, and R. Chellappa, ``The 6th AI City Challenge,'' in \emph{Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. Workshops (CVPRW)}, 2022, pp. 3346--3355, doi: 10.1109/CVPRW56347.2022.00378.

\bibitem{aic22winner}
F. Li, Z. Wang, D. Nie, S. Zhang, X. Jiang, X. Zhao, and P. Hu, ``Multi-camera vehicle tracking system for AI City Challenge 2022,'' in \emph{Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. Workshops (CVPRW)}, 2022, pp. 3265--3273, doi: 10.1109/CVPRW56347.2022.00369.

\bibitem{aic22sta}
H. Yao, Z. Duan, Z. Xie, J. Chen, X. Wu, D. Xu, and Y. Gao, ``City-scale multi-camera vehicle tracking based on space-time-appearance features,'' in \emph{Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. Workshops (CVPRW)}, 2022, pp. 3310--3318, doi: 10.1109/CVPRW56347.2022.00374.

\bibitem{mtmc2021}
K. Shim, S. Yoon, K. Ko, and C. Kim, ``Multi-target multi-camera vehicle tracking for city-scale traffic management,'' in \emph{Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. Workshops (CVPRW)}, 2021, doi: 10.1109/CVPRW53098.2021.00473.

\bibitem{scorebased}
Z.-Y. Yang, C.-W. Liu, C.-H. Chen, C.-W. Chen, and M.-C. Hsyu, ``Score-based matching for city-scale multi-target multi-camera vehicle tracking,'' \emph{Multimedia Tools Appl.}, vol. 84, no. 10, pp. 7647--7663, 2025, doi: 10.1007/s11042-024-19108-9.

\bibitem{camawarefic}
J. Zhu, Q. Li, C. Gao, Y. Ge, and K. Xu, ``Camera-aware re-identification feature for multi-target multi-camera tracking,'' \emph{Image Vis. Comput.}, vol. 142, Art. no. 104889, 2024, doi: 10.1016/j.imavis.2023.104889.

\bibitem{realtimeaccess}
O. Al\`as, R. Cervera, and R. Sanfeliu, ``A scalable real-time multi-camera vehicle tracking system for urban environments,'' \emph{IEEE Access}, 2026, early access.
