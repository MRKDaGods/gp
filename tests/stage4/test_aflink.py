...
from athar.core.data_models import GlobalTrajectory, Tracklet, TrackletFeatures, TrackletFrame
...
from athar.components.associators.aflink import aflink_post_association
...
def test_run_stage4_applies_aflink_post_association(tmp_path):
    # The v1 stage4 orchestrator is not ported (rewrite target — ROADMAP Phase 2);
    # this integration test re-enables itself when the v2 associator stage lands.
    pytest.importorskip("athar.components.associators.pipeline")
    from athar.components.associators.pipeline import run_stage4
