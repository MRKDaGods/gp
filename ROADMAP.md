...
- [x] Cross-domain eval matrix in CI — **DONE 2026-07-27** (kernel `mrkdagods/athar-joint-reid-eval`, T4; frozen `scripts/kaggle/joint_reid/results/cross_domain_matrix_20260727.json`, guard test active): deployed-mode head-to-head (CLIP norm 224, flip TTA, post-BN, SIE skipped, no rerank), joint4d vs `transreid_primary` mAP — **veri776 0.8021→0.8248 (+2.3pp), cityflow_s02 0.3004→0.4025 (+10.2pp), veriwild_3000 0.3132→0.6889 (+37.6pp)**. VehicleID column absent (encrypted upload). The joint model wins every domain incl. its rival's home turf — D4 tier-2 thesis validated.
- [x] Joint multi-domain vehicle ReID retrain on Kaggle (D4 tier 2) — **DONE 2026-07-27** (40/40 epochs over two T4 sessions, wall-guard + last.pth resume worked as designed; best VeRi mAP 0.8248/R1 0.9708 deployed-mode; ckpt `transreid_joint4d_best.pth` sha 04b3e226… = lifecycle CANDIDATE in models/reid/, promotion eval-gated). 3-DOMAIN (VeRi-776 + CityFlowV2 + VeRi-Wild = 31,431 classes; VehicleID's only Kaggle copy is password-encrypted — preflight drops it, restoring needs the official PKU password). Campaign record: `scripts/kaggle/joint_reid/README.md`. Originally: **CAMPAIGN LAUNCHED 2026-07-25** (`scripts/kaggle/joint_reid/`, design record in its README). Dataset scouting resolved ALL FOUR domains on Kaggle (the VeRi-Wild/VehicleID permission gates are bypassed by existing copies): VeRi-776 = public `abhyudaya12/...` (proven mount), VehicleID = public `maphat/vehicleid` (proven by 13v), VeRi-Wild = 23-part GDrive rar fetch (proven by July's veriwild-prep-bin; **Stage A kernel `mrkdagods/veriwild-train-prep` RUNNING** — stages the 277,797 train imgs / 30,671 ids as private dataset `mrkdagods/veriwild-train`), CityFlowV2 = GDrive + GT crops (09q recipe), **train scenes only with the S02 identity set excluded** (S02 = frozen calibration/eval scene). Stage B kernel `mrkdagods/athar-joint-reid-train` authored + locally smoked (incl. resume): canonical A5alpha TransReID ViT-B/16 CLIP recipe with three documented deviations — SIE off (deployment cams unseen; VehicleID lacks cam labels), CenterLoss off (fp16 NaN trap at ~45k classes, 09w precedent), iteration-budgeted domain-balanced epochs (1600 batches/epoch round-robin `step % 4`, single-domain P=24/K=4 PK batches, 40 epochs ≈ the A5alpha sample budget; 10.5h session guard + per-epoch resume). Push after Stage A completes.
