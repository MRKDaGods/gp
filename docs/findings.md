...
**14z verifier update (2026-05-16)**: PR #33 fixed the shared WILDTRACK ground-plane GT loader and Stage-5 prediction-frame range handling. The local GT diagnostic on `data/raw/wildtrack/annotations_positions` showed no raw density inflation (`raw_json_len == unique_pids`, 17-38 people on frames 0/200/360/399), but the defensive loader patch now dedupes repeated `personID`s and filters to calibrated in-camera projections when calibrations are available. Kaggle 14z v6 on master `bdb3fbf` moved detector-only precision/recall to healthy values (**P=94.75%, R=96.64%, FP=51, misses=32**), confirming the old 10% recall denominator failure is fixed. The verifier still fails MODA (**-1.26% vs 92.1% target**) because `evaluate_ground_plane` counts **881 ID switches** when detector-only predictions assign a fresh ID per detection; the next 14z fix should address detector-only ID/MODA accounting, not GT density or coordinate conversion.
