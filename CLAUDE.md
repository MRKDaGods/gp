...
1. **VeRi-776 ReID paper** — root `main.tex` + `references.bib`. Targets IEEE venue. Headline: two-stream fusion **93.32% mAP / 98.45% R1 @ w1=0.35** on VeRi-776 (FINALIZED 2026-06-04, triple-verified; see the FINALIZED note below). Stream 1 = TransReID ViT-B/16 CLIP (1536D in fusion); Stream 2 = CLIP-SENet ResNet101-IBN v6. Includes isolated-component ablation (A1–A5β), seed-variance, and a §Reproducibility section with SHA-pinned checkpoints. **Tie-band rule:** A5α (deployed, CE-LS+Triplet+Center) vs A5β (alternative, SupCon) differ 0.49pp mAP → within the ±0.55pp three-seed band → A5α retained & led-with.
...
`main.tex` is now fully populated with reproduced, SHA-pinned numbers and compiles clean (9 pages, all refs/cites resolve). **Headline finalized: fusion `93.32% mAP / 98.45% R1 @ w1=0.35` (1536D S1 × v6 S2)** — the fine-grid (0.05-step) optimum; the older `93.30/98.39` was the coarse-grid (0.1-step) w1=0.4 point. **Triple-verified** identical: `ali369/veri-fusion-s1dim` (dedicated fresh run) ≡ `gumfreddy/veri-fusion-boost` ≡ local CPU recompute (`scripts/_local_fusion_verify.py`). Provenance: `_repro_out/s1dim_verified.json`; full paper-table→kernel→artifact map in [docs/veri776-reproduction.md](docs/veri776-reproduction.md) §8.
- **768D vs 1536D S1 — dead heat:** 1536D 93.318/98.45 @ w1=0.35 vs 768D 93.305/98.45 @ w1=0.30 (+0.013pp). 1536D's standalone edge (89.97 vs 89.63) washes out in fusion; 1536D retained (best standalone + marginally best fused). Reported in paper §5.5.
- **Recipe narrative now LEADS with the deployed A5α** (CE-LS + Triplet(0.3) + CenterLoss, LLRD 0.75); SupCon (A5β, LLRD 0.65) is the statistically-equivalent **alternative**. Table II loss = "CE-LS + Triplet + Center".
- **Paper §4.4 Reproducibility** added: `tab:repro` with both checkpoint SHA-256 (S1 `8d32334a…`, S2 `d24bd3cd…`), two-regime guarantee (exact via frozen checkpoint / ±0.55pp via retrain), protocol assert (1678/11579/576), env torch 2.4.1+cu124 T4. Double-blind safe (no Kaggle account names in the paper).
...
- **Independent audits (×3) pass:** zero blocking issues; headline/deltas/hashes/recipe/placeholders all verified.
