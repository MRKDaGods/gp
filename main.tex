...
% ── Title & Authors ───────────────────────────────────────────────────────────
\title{93.3\% mAP on VeRi-776: Revisiting ViT Training Recipes and
...
\thanks{Corresponding author: [Name] (e-mail: [email]).}}

\markboth{IEEE TRANSACTIONS ON INTELLIGENT TRANSPORTATION SYSTEMS,~VOL.~XX,
  NO.~XX, XXXX 2026}{[AUTHOR] et al.: 93.3\% mAP on VeRi-776}
...
weights~\cite{radford2021clip}---combined with Supervised Contrastive
Loss~\cite{khosla2020supcon} and a three-step post-processing pipeline (AQE +
$k$-reciprocal re-ranking~\cite{zhong2017rerank} + flip TTA) achieves
\textbf{89.97\% mAP and 98.33\% Rank-1} on VeRi-776, the best reported result
for any pure ViT-B model on this benchmark, surpassing TransReID by
$+7.67$\,pp mAP.

Second, a training-free score-level fusion of this ViT-B stream with the
CLIP-based Semantic Enhancement Network
(CLIP-SENet~\cite{lu2025clipsenet}) achieves \textbf{93.30\% mAP and
98.45\% Rank-1}. This establishes a new state of the art on VeRi-776 in terms of mAP, while
...
\begin{itemize}
  \item \textbf{C1---Best Pure ViT-B Result:} An improved training recipe
    combining CLIP pretraining initialization, SupCon metric
    loss~\cite{khosla2020supcon}, an optimized augmentation pipeline, and AQE
    + $k$-reciprocal re-ranking~\cite{zhong2017rerank} achieves \textbf{89.97\%
    mAP / 98.33\% Rank-1} on VeRi-776---the highest reported result for any
    pure ViT-B model, surpassing CLIP-ReID (84.5\%
  ...
  \item \textbf{C2---New State of the Art in mAP:} Score-level fusion with
    CLIP-SENet~\cite{lu2025clipsenet} achieves \textbf{93.30\% mAP /
    98.45\% Rank-1}, surpassing the previous best published mAP
    (92.9\%~\cite{lu2025clipsenet}) by $\mathbf{+0.40}$\,\textbf{pp}.
  ...
  \item \textbf{C4---Public Release:} All checkpoints, evaluation notebooks,
    configuration files, and the post-processing pipeline are publicly
    released.
\end{itemize}
...
achieving 53.42\% mAP and 81.56\% Rank-1.\footnote{PROVID and VAMI values
(53.42/81.56 and 50.13/77.03) are widely cited in the survey literature;
direct verification from the original papers is recommended before camera-ready.}
...
stream in a training-free score-level fusion, adding $+0.40$\,pp mAP above the
...
for highly viewpoint-sensitive queries.
...
We adopt SupCon as the metric loss for Stream~1, replacing the soft-margin
triplet loss used in the original TransReID recipe.
...
\begin{equation}
  ...
  \label{eq:cels}
\end{equation}
where $C = 576$ training identities. The metric loss is Supervised
Contrastive Loss (SupCon)~\cite{khosla2020supcon} with $\tau = 0.07$:
\begin{equation}
  ...
  \label{eq:supcon}
\end{equation}
where $\mathcal{P}(i)$ is the set of positive pairs and $\mathcal{A}(i)$ is
all other mini-batch samples. The total objective is
$\mathcal{L} = \mathcal{L}_{\mathrm{CE\text{-}LS}} + \mathcal{L}_{\mathrm{SupCon}}$.
CircleLoss was evaluated but produced catastrophic numerical instability
(loss $\equiv \infty$ throughout training) on CLIP-pretrained ViT-B backbones
and is excluded.

\subsubsection{Augmentation Pipeline}
Table~\ref{tab:augmentation} lists the full augmentation sequence applied
during Stream~1 training. The 224\,$\times$\,224 resolution was confirmed
optimal; 256\,$\times$\,256 degraded Rank-1 by 0.12\,pp, and random
grayscale / strong hue jitter corrupted vehicle colour identity cues.
...
approximately 2.5--3.0 GPU-hours on $2\times$ Kaggle T4 GPUs with
...
Input size is $320\!\times\!320$; reducing to $256\!\times\!256$ degraded
mAP by 0.98\,pp and Rank-1 by 0.83\,pp.
...
$\to$~1536D). This configuration is optimal for Rank-1. The single-flip
CLS-only 768D configuration yields 89.97\% mAP and 98.33\% Rank-1, reported
as the primary result throughout.
...
streams have fundamentally different feature geometries (768D CLS-token
vs.\ 2048D convolutional map) and normalisation schemes; combining at the
feature level would require a learned projection prone to overfitting on
576 training identities. The optimal weights $w_1{=}0.3$, $w_2{=}0.7$
...
that mAP remains above 93.0\% across $w_1 \in [0.2,\,0.4]$, indicating
...
\begin{table}[!t]
  ...
  \begin{tabular}{L{3.2cm}L{4.5cm}}
    ...
    Component & Value \\
    \midrule
    Framework            & PyTorch 1.8.0, CUDA 11.x \\
    ...
    S1 feat.\ dim        & 768D (CLS-only, primary) \\
    S2 feat.\ dim        & 2048D \\
    AQE $k$ (S1 / S2)   & 3 / 10 \\
    Re-rank S1 (mAP)     & $k_1{=}80$, $k_2{=}15$, $\lambda{=}0.2$ \\
    Re-rank S1 (R1)      & $k_1{=}24$, $k_2{=}8$, $\lambda{=}0.2$ \\
    Re-rank S2           & $k_1{=}50$, $k_2{=}10$, $\lambda{=}0.1$ \\
    Fusion weights       & $w_1{=}0.3$, $w_2{=}0.7$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{L{2.4cm}L{1.6cm}L{3.2cm}C{1.3cm}C{1.2cm}C{1.0cm}L{1.0cm}}
    ...
    HPGN         & T-ITS 2021 & CNN+Graph                 & 80.18 & 96.72 & \xmark & \cite{shen2022hpgn} \\
    ...
    \midrule
    \textbf{Ours (S1, w/ PP)} & ---  & ViT-B/16 (CLIP init)              & \textbf{89.97} & \textbf{98.33} & \cmark & --- \\
    \textbf{Ours (Fusion)}    & ---  & ViT-B + ResNet101-IBN-a + TinyCLIP & \textbf{93.30} & \textbf{98.45} & \cmark & --- \\
    \bottomrule
  \end{tabular}
\end{table*}

The proposed ViT-B recipe surpasses all prior pure-ViT and transformer-only
methods by a substantial margin. The fusion result establishes a new absolute
state of the art, with $+0.4$\,pp mAP over CLIP-SENet~\cite{lu2025clipsenet}
and $+1.21$\,pp mAP over MBR4B-LAI~\cite{luo2023mbr4b}.
...
\begin{table}[!t]
  ...
  \begin{tabular}{L{2.4cm}L{1.4cm}L{1.9cm}C{1.0cm}C{1.0cm}}
    ...
    \midrule
    \textbf{Ours (S1, w/ PP)}                  & ---     & \textbf{ViT-B/16 (CLIP)} & \textbf{89.97} & \textbf{98.33} \\
    \bottomrule
  \end{tabular}
\end{table}
...
89.97\% mAP / 97.80\% Rank-1 result corresponds to a concat-patch flip TTA
...
a single-flip CLS-only configuration with $(k_1{=}24, k_2{=}8,
\lambda{=}0.2)$. A joint operating point at 89.71\% mAP and 98.15\% Rank-1 is
also available and used in some downstream analyses.
...
\begin{table}[!t]
  ...
  \begin{tabular}{L{4.2cm}C{1.1cm}C{1.0cm}C{1.2cm}}
    ...
    \quad+Concat-patch + flip TTA (1536D)                & 89.97 & 97.80 & $+0.69$ \\
    \midrule
...
\begin{table}[!t]
  \centering
  \caption{Fusion Weight Sensitivity on VeRi-776}
  ...
  \begin{tabular}{cccc}
    ...
    0.1 & 0.9 & 92.95 & 98.03 \\
    0.2 & 0.8 & 93.18 & 98.09 \\
    \textbf{0.3} & \textbf{0.7} & \textbf{93.30} & \textbf{98.45} \\
    0.4 & 0.6 & 93.02 & 98.45 \\
    0.5 & 0.5 & 92.47 & 98.33 \\
    0.7 & 0.3 & 91.26 & 97.97 \\
    \bottomrule
...
mAP remains above 93.0\% for $w_1 \in [0.2, 0.4]$, demonstrating robustness
...
\begin{table}[!t]
  ...
  \begin{tabular}{L{3.4cm}L{1.6cm}C{1.0cm}C{1.0cm}}
    ...
    +Re-rank                         & ViT-B      & 89.28 & 97.79 \\
    +TTA (1536D)                     & ViT-B      & 89.97 & 97.80 \\
    \midrule
    Base cosine (no PP)              & CLIP-SENet & 82.34 & 96.54 \\
    +AQE $k{=}10$                    & CLIP-SENet & 89.21 & 96.90 \\
    +Re-rank                         & CLIP-SENet & 91.54 & 97.32 \\
    \midrule
    \textbf{Full PP $\to$ Fusion}    & Both       & \textbf{93.30} & \textbf{98.45} \\
    \bottomrule
  \end{tabular}
\end{table}
...
errors in the hardest cases.

% ── Qualitative error-mode table (replaces retrieval panels until camera-ready) ──
\begin{table}[!t]
  ...
  {\footnotesize Success cases are included for contrast.
   ...
   Full retrieval panel figures are available in the project repository
...
with DINOv2~\cite{oquab2023dinov2}---a self-supervised alternative---underperforms
CLIP-pretrained ViT-B by 0.70\,pp mAP on VeRi-776 (DINOv2 post-rerank:
89.27\% vs.\ ours: 89.97\%), confirming that CLIP's \emph{semantic
supervision}---not merely pretraining scale---is the key factor. SupCon loss,
...
The optimal $w_2{=}0.7$ for CLIP-SENet reflects its higher standalone
post-processed performance (91.54\% vs.\ 89.97\% mAP for ViT-B). The ViT-B
branch still contributes $+1.76$\,pp above CLIP-SENet alone, confirming
...
Despite its larger parameter count, it overfit VeRi-776's 576 training
identities, reaching only 87.95\% mAP post-rerank---below the ViT-B CLIP
result. This motivates systematic post-processing over backbone scaling as
the primary performance lever for small-scale ReID benchmarks.
...
\begin{enumerate}
  ...
  \item All results are reported on VeRi-776 under the standard supervised
    ...
    required for MTMC.
  \item Per-component isolated ablations (CLIP initialisation alone, SupCon
...
training recipe---combining CLIP pretraining initialisation, SupCon metric
loss, an optimised augmentation pipeline, AdamW optimisation, and a three-step
post-processing pipeline---achieves 89.97\% mAP and 98.33\% Rank-1, the best
reported result for any pure ViT-B model on this benchmark, surpassing
TransReID by $+7.67$\,pp mAP~\cite{he2021transreid}. Score-level fusion with
a CLIP-SENet branch~\cite{lu2025clipsenet} achieves 93.3\% mAP and 98.45\%
Rank-1, a new state of the art on VeRi-776 in terms of mAP, exceeding the
previous best published mAP (CLIP-SENet standalone: 92.9\%~\cite{lu2025clipsenet})
by $+0.40$\,pp. Rank-1 accuracy (98.45\%) is competitive with but does not
