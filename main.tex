...
loss) on CLIP-pretrained ViT-B backbones in earlier experiments and is
excluded; a SHA-pinned reproduction of this divergence is pending.
...
provenance mapping are released, anonymized, in the supplementary materials.
...
observed to underperform CLIP-pretrained ViT-B in earlier experiments
(DINOv2 post-rerank mAP: ---, SHA-pinned reproduction pending; ours:
89.97\%), suggesting that CLIP's \emph{semantic supervision}---not merely
...
576 training identities, falling below the ViT-B CLIP result post-rerank
(ViT-L/14 post-rerank mAP: ---, SHA-pinned reproduction pending). This
motivates systematic post-processing over backbone scaling as
