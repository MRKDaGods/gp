...
and 97.80\% Rank-1} at its best-mAP operating point on VeRi-776, with a
...
\begin{itemize}
  \item \textbf{C1---Best Pure ViT-B Result:} An improved training recipe
    ...
    mAP / 97.80\% Rank-1} at the best-mAP operating point on VeRi-776---the
...
all other mini-batch samples. For the paper-recipe variant (A5$\beta$), the
total objective is
$\mathcal{L} = \mathcal{L}_{\mathrm{CE\text{-}LS}} + \mathcal{L}_{\mathrm{SupCon}}$.
The currently deployed checkpoint (A5$\alpha$, actual recipe) was trained with
CE-LS + TripletLoss($m{=}0.3$) + CenterLoss($\lambda{=}5\times10^{-4}$,
...
catastrophic numerical instability (loss $\equiv \infty$ throughout training)
on CLIP-pretrained ViT-B backbones and is excluded.
...
(89.97\% mAP and 97.80\% Rank-1), while the single-flip CLS-only 768D
configuration yields the best-Rank-1 operating point (85.14\% mAP and
98.33\% Rank-1). A joint operating point of 89.71\% mAP and 98.15\% Rank-1
is used only where a single balanced Stream~1 setting is required.
...
\begin{table*}[!t]
  ...
  \begin{tabular}{L{2.4cm}L{1.6cm}L{3.2cm}C{1.3cm}C{1.2cm}C{1.0cm}L{1.0cm}}
    ...
    \midrule
    \textbf{Ours (S1, w/ PP)} & ---  & ViT-B/16 (CLIP init)              & \textbf{89.97} & \textbf{97.80} & \cmark & --- \\
...
the art in mAP on VeRi-776, with $+0.4$\,pp mAP over CLIP-SENet~\cite{lu2025clipsenet}
...
\begin{table}[!t]
  ...
  \begin{tabular}{L{2.4cm}L{1.4cm}L{1.9cm}C{1.0cm}C{1.0cm}}
    ...
    \midrule
    \textbf{Ours (S1, w/ PP)}                  & ---     & \textbf{ViT-B/16 (CLIP)} & \textbf{89.97} & \textbf{97.80} \\
    \bottomrule
  \end{tabular}
\end{table}
...
mAP and 97.80\% Rank-1 at its best-mAP operating point, with a separate
